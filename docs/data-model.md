...
{"id":"c3d4e5f60718293a4b5c6d7e8f90a1b2","type":20,"status":2,"ts":1755500910,"chat_id":111111111,"username":"mario_rossi"}
```

| Field | Meaning |
|---|---|
| `id` | 32-hex-char event ID (16 random bytes from `esp_random()`, no dashes). `START`/`END` rows of the same duration event share an `id`. |
| `type` | `EventType` enum value — see the table in [`src/events/EventTypes.h`](../src/events/EventTypes.h) (`REBOOT=0`, `POWER_LOSS=1`, `NETWORK_ISSUE=2`, `ALARM_GENERAL=10`, `ALARM_INTERNAL=11`, `ALARM_GARAGE=12`, `ARM_GENERAL=20`, `DISARM_GENERAL=21`, `ARM_INTERNAL=22`, `DISARM_INTERNAL=23`, `ARM_GARAGE=24`, `DISARM_GARAGE=25`). Values are never reassigned; new types are appended. |
| `status` | `0`=`START`, `1`=`END`, `2`=`INSTANT` (single-row events like `REBOOT` and the `ARM_*`/`DISARM_*` remote-command types). |
...
| `chat_id`/`username` | Present only on a row generated by a remote `/setalarm` command (see [commands.md](commands.md)) — the requester's chat_id and a snapshot of their username at request time (not re-resolved later). Omitted entirely for every other row, so this costs nothing on the common case. |
