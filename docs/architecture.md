...
`REBOOT` is a special case: `setup()` waits up to `kRebootNtpWaitTimeoutMs` (20 s) for a real NTP sync before logging/notifying the event, so it can carry a real timestamp instead of always being approximate — with the same fallback to the NVS anchor if the timeout elapses (WiFi down or slow).

## Network: reachability beyond WiFi association

WiFi being associated doesn't mean Telegram is reachable (router up, ISP line down is the common case). Rather than adding a dedicated probe, `NETWORK_ISSUE` detection (sec. 3.4.1) reuses the outcome already produced by every real Telegram send: `TelegramReachabilityTracker` (see [`src/network/TelegramReachability.h`](../src/network/TelegramReachability.h)) watches for consecutive `TRANSIENT_NETWORK` classifications (connection-level failures, as opposed to an application error like 429/403 which proves the link works) and flips to "unreachable" after 3 in a row. `loop()` combines `isWifiConnected() && isTelegramReachable()` as the signal fed to `NetworkIssueTracker`; WiFi down alone still dominates, since no send is possible either way. This signal only refreshes on an actual send attempt (notification or command reply) — during pure silence it goes stale — but that's exactly the case that matters: an alarm firing always attempts a send.
