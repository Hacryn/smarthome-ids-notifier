...
| Command | Effect |
|---|---|
| `/log [n]` | Shows the last `n` aggregated events (`START`/`END` pairs sharing the same event ID, merged with a computed duration), most recent first. Default `n` is 10, capped at 50. Still-open events show `APERTO`; approximate timestamps are prefixed `~`. A row from a remote `/setalarm` command shows the requester's chat_id/username inline. |
| `/status` | Firmware version, uptime (`<d>g <h>h <m>m <s>s`) and last reset cause, alarm state per type (`ALARM_GENERAL`/`ALARM_INTERNAL`/`ALARM_GARAGE`, from the logged open event rather than the raw pin level), 230V mains state (from the `POWER_LOSS` open event), WiFi status (SSID/RSSI/assigned IP and whether it's static or DHCP, or backoff attempt if disconnected), Telegram API reachability (retrospective — reflects the outcome of the last sends attempted, not the instant `/status` itself was run; see [architecture.md](architecture.md#network-reachability-beyond-wifi-association)), NTP sync status, open-event count, pending/abandoned notification counts and retry-timer state, LittleFS usage and write-error count (with degraded-mode flag), ISR queue overflow count, last rotation timestamp (formatted per the caller's preferences), last system-level send error. |
...
| `/settimezone <preset>` | Sets your own timezone. `<preset>` is one of the names in [`src/config/TimezonePresets.h`](../src/config/TimezonePresets.h): `UTC`, `Europe/Rome` (default), `Europe/Berlin`, `Europe/London`, `Europe/Moscow`, `America/New_York`, `America/Los_Angeles`. |
| `/notify <type> on\|off` | Enables/disables notifications for one event type, for you only — the event is still always written to the log. `<type>` is the type's `commandName` from `EVENT_TYPES`: `REBOOT`, `POWER_LOSS`, `NETWORK_ISSUE`, `ALARM_GENERAL`, `ALARM_INTERNAL`, `ALARM_GARAGE`, `ARM_GENERAL`, `DISARM_GENERAL`, `ARM_INTERNAL`, `DISARM_INTERNAL`, `ARM_GARAGE`, `DISARM_GARAGE`. |
| `/setalarm <GENERALE\|INTERNO\|GARAGE> <ON\|OFF>` | Arms (`ON`) or disarms (`OFF`) the given zone remotely by pulsing the matching output pin (see [hardware-setup.md](hardware-setup.md#remote-armdisarm-output-pins)). Available to any whitelisted user, not just admins. Rejected with an explicit reply if a previous command's pulse is still in progress. Only confirms the command was sent — the panel doesn't report back whether it actually changed state. Logged and notified like any other event; `/log` shows who requested it. |
